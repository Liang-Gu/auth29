...
import com.wbst.mapper.WhiteAuthMapper;
...
@Service
public class WhiteAuthServiceImpl implements WhiteAuthService {


    @Resource
    private WhiteAuthMapper whiteAuthMapper;

    @Reference
    private PersonService personService;

    @Autowired
    private WhiteAuthLogService WhiteAuthLogService;
    ...
    public void add(AuthQuery authQuery) {
        ...
        for (String cardNum : cardNums) {
            ...
            for (AuthUtil authUtil : authUtils) {
                //根据通行权限和授权编号查询数据
                Auth auth = whiteAuthMapper.findByCardNumAndaccessLevelId(cardNum, authUtil.getAccessLevelId());
                //判断对象是否有数据
                if(auth!=null){//数据库有数据
                    //操作日志 操作内容
                    ...
                    if(authUtil.getAccessLevelStatus()==0){//永久权限
                        //修改数数据
                        whiteAuthMapper.update(cardNum,authUtil.getAccessLevelId(),null,null,0);
                        ...
                        WhiteAuthLogService.saveOperation(authLog);
                        //删除lenel数据
...
//                   lenelAuth(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
                        ...
                        if(auth.getAccessLevelStatus()==0 ||auth.getEffectiveTimeEnd().after(authUtil.getEffectiveTimeEnd())){
                            ...
                            whiteAuthMapper.update(cardNum,authUtil.getAccessLevelId(),authUtil.getEffectiveTimeStart(),authUtil.getEffectiveTimeEnd(),1);
...
//                   lenelAuth(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
                    ...
                    whiteAuthMapper.insert(cardNum,authUtil.getAccessLevelId(),authUtil.getEffectiveTimeStart(),authUtil.getEffectiveTimeEnd(),authUtil.getAccessLevelStatus());
                    String operationContent=null;
                    if(authUtil.getAccessLevelStatus()==1){//临时卡
                        operationContent="新增"+authUtil.getAccessLevelName()+"通行权限，有效时间为："+authUtil.getEffectiveTimeStart()+"至"+authUtil.getEffectiveTimeEnd();
                    }else {
...
//                   lenelAuth(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
    ...
    public PageList<AccessLevel> findAllAccessLevel(AccessLevelQuery accessLevelQuery) {
        ...
        List<AccessLevel> accessLevelList= whiteAuthMapper.findAllAccessLevel(accessLevelQuery);
    ...
    public List<AccessLevel> findCommon(AuthQuery authQuery) {
        ...
        //查询在授权表中查询共有权限
        List<AccessLevel> accessLevelIds=whiteAuthMapper.findCommon(authQuery.getCardNums(),size);
    ...
    public void batchUpdate(AuthQuery authQuery) {
        ...
        for (AuthUtil authUtil : authUtils) {
            if(authUtil.getAccessLevelStatus()==0){//修改为永久权限
                for (String cardNum : cardNums) {
                    whiteAuthMapper.updateForever(cardNum,authUtil.getAccessLevelId());
...
//                   lenelAuth(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
                ...
                Auth auth = whiteAuthMapper.findByCardNumAndaccessLevelId(cardNums.get(0), authUtil.getAccessLevelId());
                if(auth.getAccessLevelStatus()==0 ||auth.getEffectiveTimeEnd().after(authUtil.getEffectiveTimeEnd())){
                    continue;
                }else {
                    whiteAuthMapper.batchUpdate(cardNums,authUtil.getAccessLevelId(),authUtil.getEffectiveTimeStart(),authUtil.getEffectiveTimeEnd(),1);
...
//                   lenelAuth(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
    ...
    public void batchDelete(AuthQuery authQuery) {
        ...
        for (String cardNum : cardNums) {
            Person byCardNum = personService.findByCardNum(cardNum);
            whiteAuthMapper.batchDelete(cardNum,authUtils);
            for (AuthUtil authUtil : authUtils) {
...
//          deleteLenel(cardNum.toString(),auth.getAccessLevelId().toString(),auth.getEffectiveTime_start(),auth.getEffectiveTime_end());
    ...
    public PageList<AuthLog> findAllAuthLog(AuthLogQuery authlogQuery) {
        return WhiteAuthLogService.findAllAuthLog(authlogQuery);
    }
